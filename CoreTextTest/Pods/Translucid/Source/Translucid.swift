...
public class Translucid: UIView {
    ...
    public override var frame: CGRect {
        didSet {
            self.imageLayer.frame = CGRect.init(x: 0.0, y: 0.0, width: self.bounds.size.width, height: self.bounds.size.height+200.0)
    ...
    public func animate() {
        ...
        animation.toValue = NSValue(cgPoint:CGPoint.init(x: self.imageLayer.position.x, y: self.imageLayer.position.y-200))
    ...
    private func autoResizeTextLayer() {
      ...
      //  let option :NSStringDrawingOptions =   .usesFontLeading
        ...
        var rect: CGRect = NSString(string: self.text).boundingRect(with:         CGSize.init(width: self.bounds.width, height: CGFloat(MAXFLOAT)), options: option , attributes: [NSFontAttributeName: self.font.withSize(fontSize)], context: nil)
        
        while rect.size.height <= self.bounds.size.height {
            fontSize += 1
            rect = NSString(string: self.text).boundingRect(with:CGSize.init(width: self.bounds.width, height: CGFloat(MAXFLOAT)), options: option, attributes: [NSFontAttributeName: self.font.withSize(fontSize)], context: nil)
        }
